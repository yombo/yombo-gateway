...
def route_api_v1_server(webapp):
    with webapp.subroute("/api/v1") as webapp:
        ...
        def apiv1_server_commands_index(webinterface, request, session):
            ...
            url = f"/v1/command?_pagestart={offset}&_pagelimit={limit}"
            if search is not None:
                url = url + f"&?_filters[label]*{search}&_filters[description]*{search}" \
                            f"&_filters[machine_label]*{search}&_filteroperator=or"
        ...
        def apiv1_server_dns_check_available(webinterface, request, session, dnsname):
            ...
            url = f"/v1/dns/check_available/{dnsname}"
        ...
        def apiv1_server_devicetypes_index(webinterface, request, session):
            ...
            url = f"/v1/device_type?_pagestart={offset}&_pagelimit={limit}"
            if search is not None:
                url = url + f"&?_filters[label]*{search}&_filters[description]*{search}" \
                            f"&_filters[machine_label]*{search}&_filteroperator=or"
        ...
        def apiv1_server_inputtypes_index(webinterface, request, session):
            ...
            url = f"/v1/input_type?_pagestart={offset}&_pagelimit={limit}"
            if search is not None:
                url = url + f"&?_filters[label]*{search}&_filters[description]*{search}" \
                            f"&_filters[machine_label]*{search}&_filteroperator=or"
        ...
        def apiv1_server_modules_index(webinterface, request, session):
            ...
            url = f"/v1/module?_pagestart={offset}&_pagelimit={limit}"
            if search is not None:
                url = url + f"&?_filters[label]*{search}&_filters[short_description]*{search}" \
                            f"&_filters[machine_label]*{search}&_filteroperator=or"
