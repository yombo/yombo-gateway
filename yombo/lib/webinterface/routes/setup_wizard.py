...
def route_setup_wizard(webapp):
    with webapp.subroute("/setup_wizard") as webapp:
        ...
        def page_setup_wizard_select_gateway(webinterface, request, session):
            ...
            page = webinterface.get_template(request, webinterface.wi_dir + "/pages/setup_wizard/select_gateway.html")
        ...
        def page_setup_wizard_advanced_settings_post(webinterface, request, session):
            ...
            for gateway_id, gateway in available_gateways.items():
                ...
                if gateway['label'].lower() == submitted_gateway_label.lower():
                    valid_submit = False
                    webinterface.add_alert("There's already a gateway with this label.")
                if gateway['machine_label'].lower() == submitted_gateway_machine_label.lower():
                    valid_submit = False
                    webinterface.add_alert("There's already a gateway with this machine label.")
        ...
        def page_setup_wizard_advanced_settings_show_form(webinterface, request, session):
            ...
            page = webinterface.get_template(request, webinterface.wi_dir + "/pages/setup_wizard/advanced_settings.html")
            return page.render(
                               alerts=webinterface.get_alerts(),
        ...
        def page_setup_wizard_dns_get(webinterface, request, session):
            if session.get("setup_wizard_last_step", 1) not in ("advanced_settings", "dns", "finished"):
                webinterface.add_alert("Invalid wizard state. Please don't use the browser forward or back buttons. uu")
                return webinterface.redirect(request, "/setup_wizard/select_gateway")
        ...
        def page_setup_wizard_dns_post(webinterface, request, session):
            ...
            """
            if session.get("setup_wizard_last_step", 1) not in ("advanced_settings", "dns", "finished"):
                webinterface.add_alert("Invalid wizard state. Please don't use the browser forward or back buttons. ii")
                return webinterface.redirect(request, "/setup_wizard/select_gateway")
            ...
            if session["setup_wizard_gateway_id"] == "new":
                ...
                data = response.content["data"]
                session["setup_wizard_gateway_id"] = data["attributes"]["id"]
        ...
        def form_setup_wizard_dns(webinterface, request, session):
            ...
            page = webinterface.get_template(request, webinterface.wi_dir + "/pages/setup_wizard/dns.html")
            return page.render(
                alerts=webinterface.get_alerts(),
                dns_fqdn=dns_fqdn,
                dns_name=dns_name,
        ...
        def page_setup_wizard_finished_get(webinterface, request, session):
            page = webinterface.get_template(request, webinterface.wi_dir + "/pages/setup_wizard/finished.html")
    ...
    #            return webinterface.redirect(request, "/setup_wizard/gpg")
