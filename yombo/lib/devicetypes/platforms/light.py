...
:view-source: `View Source Code <https://yombo.net/docs/gateway/html/current/_modules/yombo/lib/devicetypes/platforms/light.html>`_
"""
...
from yombo.constants.state_extra import STATE_EXTRA_BRIGHTNESS, STATE_EXTRA_RGB_COLOR
...
class Light(Device):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.MACHINE_STATE_EXTRA_FIELDS[STATE_EXTRA_BRIGHTNESS] = True
        self.MACHINE_STATE_EXTRA_FIELDS[STATE_EXTRA_RGB_COLOR] = True
    ...
    def set_brightness(self, brightness, **kwargs):
        ...
        print(f"setting device brightness: {command} - {kwargs}")
    ...
    def brightness(self):
        ...
        if len(self.state_history) > 0:
            ...
            if STATE_EXTRA_BRIGHTNESS in machine_state_extra:
                return machine_state_extra[STATE_EXTRA_BRIGHTNESS]
            else:
                return 0
        return None
    ...
    def percent(self):
        ...
        if len(self.state_history) > 0:
            machine_state_extra = self.state_history[0].machine_state_extra
            if STATE_EXTRA_BRIGHTNESS in machine_state_extra:
                return translate_int_value(machine_state_extra[STATE_EXTRA_BRIGHTNESS],
                                           0, self.FEATURES[FEATURE_NUMBER_OF_STEPS],
                                           0, 100)
            else:
                return 0
    ...
    def rgb_color(self):
        ...
        if len(self.state_history) > 0:
            machine_state_extra = self.state_history[0].machine_state_extra
            if STATE_EXTRA_RGB_COLOR in machine_state_extra:
                rgb = machine_state_extra[STATE_EXTRA_RGB_COLOR]
                return rgb
            else:
                return 0, 0, 0
        return None
    ...
    def status_attributes(self):
        ...
        """
        data = {}
        state = self.state_history[0].machine_state_extra
        if self.is_on:
    ...
    def generate_human_state(self, machine_state, machine_state_extra):
        return str(self.calc_percent(machine_state, machine_state_extra)) + "%"

    def generate_human_message(self, machine_state, machine_state_extra):
        return f"{self.area_label} is now {self.calc_percent(machine_state, machine_state_extra)}%"
