...
:view-source: `View Source Code <https://yombo.net/docs/gateway/html/current/_modules/yombo/lib/devicetypes/platforms/camera.html>`_
"""
...
from yombo.constants.state_extra import STATE_EXTRA_VALUE__IDLE, STATE_EXTRA_VALUE__RECORDING
...
logger = get_logger("lib.devicetypes.platforms.camera")
...
class Camera(Device):
    ...
    def toggle(self):
        if self.state_history[0].machine_state == STATE_EXTRA_VALUE__IDLE:
            return self.command(COMMAND_RECORD)
        elif self.state_history[0].machine_state == STATE_EXTRA_VALUE__RECORDING:
    ...
    def get_camera_image(self):
        ...
        if self.image_url is not None:
            ...
            image = ImageContainer(content=image_results.content,
                                   content_type=image_results.headers['content-type'][0])
            return image
        else:
