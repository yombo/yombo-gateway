...
from yombo.lib.gateways.mqtt import Gateway_Coms
...
class Gateways(YomboLibrary):
    ...
    def _init_(self, **kwargs):
        ...
        if self.master_gateway() not in self.gateways:
            ...
            self._Configs.set('core', 'master_gateway', gateway_id)
            self._AMQPYombo.send_local_information(full=True)

        if self.is_master() is True:
            self.master_mqtt_host = 'i.' + self.gateways[gateway_id].fqdn
            self.master_mqtt_ssl = False
            self.master_mqtt_port = self.gateways[gateway_id].internal_mqtt
            self.master_mqtt_port_ssl = self.gateways[gateway_id].internal_mqtt_le
            self.master_websock_ssl = False
            self.master_websock_port = self.gateways[gateway_id].internal_mqtt_ws
            self.master_websock_port_ssl = self.gateways[gateway_id].internal_mqtt_ws_le
        else:
        ...
        self.coms = Gateway_Coms(self)

    def _start_(self, **kwargs):
        self.library_phase = 3
        if self._Loader.operating_mode != 'run':
            return
        self.mqtt = self._MQTT.new(mqtt_incoming_callback=self.coms.mqtt_incoming,
    ...
    def _started_(self, **kwargs):
        ...
        if self._Loader.operating_mode != 'run':
            return
        self.coms.publish_data('gw', "all", "lib/gateway/online", "")
        reactor.callLater(3, self.coms.send_all_info, set_ok_to_publish_updates=True)
    ...
    def _stop_(self, **kwargs):
        ...
        if hasattr(self, 'mqtt'):
            if self.mqtt is not None:
                self.coms.publish_data('gw', "all", "lib/gateway/offline", "")
                yield sleep(0.05)
    ...
    def ping_gateways(self):
        ...
        for gateway_id, gateway in self.gateways.items():
            ...
            request_id = self.coms.publish_data('req', gateway_id, "system/ping", message)
    ...
    def _atoms_set_(self, **kwargs):
        ...
        self.coms.publish_data('gw', 'all', "lib/atoms", atom)
    ...
    def _device_command_(self, **kwargs):
        ...
        self.coms.publish_data('gw', 'all', topic, message)
    ...
    def _device_command_status_(self, **kwargs):
        ...
        self.coms.publish_data('gw', 'all', "lib/device_command_status", message)
    ...
    def _device_status_(self, **kwargs):
        ...
        self.coms.publish_data('gw', 'all', topic, {device_id: device.status_all.asdict()})
    ...
    def _states_set_(self, **kwargs):
        ...
        self.coms.publish_data('gw', 'all', "lib/states", state)
