# This file has been modified from it's original by Mitch Schwenk to support
# the requirements of the Yombo Gateway.
#
...
from yombo.ext.txrdq.dpq import DeferredPriorityQueue
from yombo.ext.txrdq.pool import DeferredPool
from yombo.ext.txrdq.job import Job

# Yombo Items
from yombo.core.log import get_logger
logger = get_logger('ext.txrdq.rdq')
...
class ResizableDispatchQueue(object):
    ...
    def __init__(self, func, width=0, size=None, backlog=None, name=None, save_on_exit=None):
        self._func = func
    ...
    def put(self, jobarg, done_callback=None, done_arg=None, priority=None):
        """
        ...
        if self.stopped:
            ...
            job = Job(jobarg, priority, done_callback, done_arg)
    ...
    def stop(self, cancelUnderway=False):
        ...
        logger.debug('Stop called. Underway in queue %r' % self._underway)
        ...
        if cancelUnderway:
            ...
            underwayJobs = []
        logger.debug('stop called. underwayJobs is %r' % underwayJobs)
        d = self._drain()
